...
pub fn create_directory(path: &PathBuf) -> Result<(), Ourerror>{
    match create_dir_all(path) {
       Ok(_) => Ok(()),
        Err(e) => {log::error!("Cannot create directory {}; {}", path.to_str().unwrap(),e);
...
pub fn random_string(n: usize) -> String{
    ...
    let name: String = (0..n)
     .map(|_|{let idx = rng.gen_range(0..CHARSET.len());  CHARSET[idx] as char}).collect();
    name
}
...
pub fn mount_directory(path: Option<&PathBuf>, mount_point: &PathBuf, flags: Vec<MsFlags>) -> Result<(), Ourerror>{    
